...
•	Потоки тяжёлые (создание ~1 МБ стека, переключение контекста занимает десятки микросекунд).

•	Когда поток ждёт (например, URLSession ждёт ответа), он просто висит — ничего не делает, но занимает системные ресурсы.

•	Из-за этого может произойти “thread explosion”: тысячи задач → сотни потоков → лаги и перерасход ресурсов.
...
•	Есть ограниченное количество потоков (примерно столько, сколько ядер CPU).

•	Задачи (Task) делят эти потоки между собой.

•	Когда одна задача “засыпает” (ждёт IO, сетевой ответ и т.п.), поток не простаивает, а берёт другую задачу.

<img src="https://github.com/sashakid/ios-guide/blob/master/Images/continuation.png">

Что реально происходит:

1.	Компилятор разворачивает этот код в state machine (автомат состояний).

2.	Когда выполнение доходит до await, создаётся continuation — это объект, который сохраняет всё состояние функции:

•	где именно мы остановились;

•	какие переменные есть в контексте;

•	что нужно сделать, когда результат придёт.

3.	После этого поток освобождается — он может заняться другой задачей.

4.	Когда ответ готов — runtime возобновляет continuation, т.е. возвращается в сохранённую точку и продолжает код на любом свободном потоке.

__Почему это быстрее:__

1.	Нет переключения потоков (context switch) в привычном смысле.

• Continuation — это чисто логический переключатель, выполняемый в памяти, а не на уровне ОС.

• Переключение потока ОС → ~10–100 мкс

• Возобновление continuation → <1 мкс

2.	Нет “thread explosion” — потоков мало, и они постоянно заняты полезной работой.

3.	Эффективная работа с памятью — вместо большого стека у потока, continuation хранит только нужное состояние функции.

Пример с ручным continuation:
...
Итог:
- На serial очереди любой внутренний sync на ту же очередь всегда deadlock.
- На concurrent очереди внутренний sync может не вызвать дедлок, если система запускает блок на другом потоке.
- Но если вызываешь sync на текущем потоке и ждёшь выполнения блока на той же concurrent очереди → всё равно возможен deadlock.
...
Ты можешь вызывать serial.async откуда угодно — даже из разных потоков — и быть уверенным, что задачи будут выполняться последовательно.
...
❗ Почему UIKit работает только на главном потоке?

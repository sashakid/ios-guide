- [Общие вопросы](#общие-вопросы)
  ...
  - [Вопросы про таймер и runloop](#вопросы-про-таймер-и-runloop)
...
-	for — когда знаешь заранее, сколько раз крутиться
-	while — когда не знаешь, цикл крутится пока условие истинно
...
- you absolutely have to; and
- you are willing to have the calling thread blocked until the sync task runs.
...
**Краткая таблица с последовательностью этапов**

| Этап | Описание | Примечание |
| ---- | -------- | ---------- |
| 1. Построение UI | В приложении определяется структура пользовательского интерфейса с помощью UIView и CALayer. | На этом этапе происходит создание объектов UI и их свойств. |
| 2. Подготовка к рендеру |	UIKit и Core Animation собирают данные о слоях (views и sublayers) в дерево отображения (Layer Tree). | Это дерево содержит все визуальные объекты и их свойства, но не связано с конкретным экраном или устройством. |
| 3. Layout | Система рассчитывает размеры и положение всех элементов интерфейса. | Вызываются методы `layoutSubviews` и `sizeThatFits`. |
| 4. Отрисовка (Drawing) | Слои создают свои содержимое (например, изображения, текст, тени) через методы `draw(_:)` или с помощью Core Graphics. | Если вы рисуете вручную, это также происходит на этом этапе. |
| 5. Подготовка слоев | Создается Rendering Tree на основе Layer Tree, где исключены элементы вне видимой области. | Оптимизация для исключения невидимых или перекрытых объектов. |
| 6. Композиция (Compositing) | Rendering Tree передается в Metal/OpenGL, где каждый слой преобразуется в текстуру. | На этом этапе применяются эффекты (маски, тени) и вычисляются матрицы трансформации для каждого слоя. |
| 7. Offscreen Rendering | Если некоторые слои требуют предварительных вычислений, они обрабатываются в offscreen buffer. | Например, скругленные углы с `masksToBounds`, сложные градиенты или тени. |
| 8. Рендеринг | Финальные текстуры объединяются GPU в Render Pass, чтобы создать финальное изображение. | Этот процесс выполняется в графическом процессоре для минимизации задержек. |
| 9. Отображение на экране | Финальный кадр передается на дисплей через VSync (синхронизацию с частотой обновления экрана). | Это обеспечивает плавность и отсутствие разрывов экрана. |

https://www.objc.io/issues/3-views/moving-pixels-onto-the-screen/

https://www.youtube.com/watch?v=TQRJXLXhG2o

https://www.youtube.com/watch?v=Qusz9R39ndw

<a name="VSYNC"></a>

## Что такое VSYNC?

**VSYNC (Vertical Synchronization)** — это механизм, который синхронизирует отрисовку графики с частотой обновления экрана устройства или, другими словами, дедлайн для каждой фазы нашего рендер лупа. Каждый VSYNC — меняет нам следующий кадр. Для достижения лучшей оптимизации каждый фрейм распараллеривается. Пока CPU читает кадр номер `N`, в это время GPU рендерит предыдущий кадр `N-1`. В iOS он используется для обеспечения плавности анимаций и предотвращения артефактов, таких как разрывы экрана (screen tearing), которые могут возникнуть, если графика обновляется быстрее или медленнее, чем экран может отобразить.

Как работает VSync в iOS?

1.	Частота обновления экрана:
- На большинстве iOS-устройств экран обновляется с частотой 60 Гц (или 120 Гц на устройствах с ProMotion). Это означает, что экран обновляется каждые 16.67 мс (или 8.33 мс для 120 Гц).
2.	Рендеринг графики:
- Core Animation и другие графические подсистемы в iOS синхронизируются с VSync, чтобы кадры анимации и графики обновлялись только в начале каждого нового цикла обновления экрана. Это предотвращает появление частично обновленных кадров.
3.	Основной цикл отрисовки:
- iOS использует CADisplayLink, который синхронизируется с VSync. Он сообщает приложению, когда начинается следующий цикл обновления экрана, чтобы анимации и рендеринг происходили вовремя.

Почему VSync важен?

1.	Плавность анимаций:
Если кадры обновляются с той же частотой, что и экран, анимации выглядят плавно и предсказуемо.
2.	Отсутствие разрывов экрана:
Разрывы экрана происходят, если частота обновления графики не совпадает с частотой обновления экрана. VSync предотвращает это.
3.	Стабильная производительность:
Ограничивая частоту обновления графики до частоты обновления экрана, VSync помогает избежать перегрузки GPU и экономить энергию.

Недостатки VSync:

1.	Задержка:
Если кадр не успевает быть отрендерен к началу следующего обновления экрана, он будет отложен до следующего цикла. Это вызывает задержки и может привести к дерганью (stutter).
2.	Пропуск кадров:
В случаях высокой нагрузки GPU может не успевать обрабатывать графику в реальном времени, из-за чего пропускаются кадры.

Вывод

VSync в iOS — это важная часть обеспечения плавной и качественной графики. Однако для сложных графических задач разработчики должны оптимизировать свои приложения, чтобы избегать пропусков кадров и излишней нагрузки на GPU.
...
`autoreleasepool` — это контейнер, который откладывает освобождение autoreleased-объектов до конца блока, а не до конца run loop.
- Без `autoreleasepool` такие объекты живут до следующего цикла run loop
- С `autoreleasepool` — освобождаются сразу при выходе из блока

## Вопросы про таймер и runloop

NSTimer и поток

__Если создать Timer.scheduledTimer(...) на фоновом потоке, будет ли он работать?__

Подвох:
- scheduledTimer автоматически добавляется в current run loop текущего потока в default mode.
- На главном потоке run loop есть и он работает постоянно → таймер будет срабатывать.
- На фоновых потоках run loop не всегда запущен. Если не запустить RunLoop.current.run(), таймер не сработает.

Что проверяют: знание того, что таймер зависит от run loop, а run loop на фоновых потоках не запущен автоматически.
___

CADisplayLink и RunLoopMode

__Почему CADisplayLink может «залипать» или не обновляться во время скролла?__

Подвох:
- CADisplayLink привязан к run loop mode, обычно .default.
- UIScrollView во время скролла переключает run loop в .tracking.
- Если не добавить displayLink в .common modes, он «замрет» на время скролла.

Проверяют: понимание run loop modes.
___
DispatchTimer vs NSTimer

__Почему таймер на GCD (DispatchSourceTimer) работает на фоновом потоке, даже если run loop не запущен?__

Подвох:
- GCD таймер не зависит от run loop — он использует собственный механизм планирования.
- В отличие от NSTimer, его можно использовать в любом потоке и он будет срабатывать.

Проверяют: разницу между run loop–зависимыми и GCD таймерами.
___
Таймеры и многопоточность

__Если вы создали Timer на фоне и хотите обновить UI, что может пойти не так?__

Подвох:
- Timer срабатывает на том потоке, где он добавлен в run loop.
- Попытка обновить UI с фонового потока → крэш, нужно делать DispatchQueue.main.async { … }.
___
Таймер и modes

__Что произойдет, если добавить Timer в run loop с .default mode и сразу же вызвать scrollView.setContentOffset?__

Подвох:
- Во время скролла run loop переключается в .tracking mode → таймер временно не сработает.
- Проверяют знание subtle behavior run loop modes.
___
Run loop и задержки

__Если выполнить тяжелую синхронную задачу на главном потоке, будут ли срабатывать таймеры и анимации?__

Подвох:
- Главный run loop блокируется, ни один Timer, ни CADisplayLink, ни анимации не будут срабатывать, пока блокировка не закончится.
- Это проверка понимания того, что таймеры и анимации не работают «параллельно», а зависят от run loop.
